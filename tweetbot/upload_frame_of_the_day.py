...
starting_frame = int(os.listdir('/root/start')[0])

index = os.listdir('/root/index/')[0]
next_frame = int(index)+1
# upload next frame
print ("next frame "+str(next_frame))

image_folder = "{0:04d}".format(math.floor((next_frame+starting_frame)/10000))


image_path = "/root/files/"+image_folder+"/"+"{0:08d}.jpg".format(next_frame+starting_frame)
